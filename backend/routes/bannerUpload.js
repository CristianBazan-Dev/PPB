...
router.post("/upload/banner", auth, authAdmin, (req, res) => {
    try{
        ...
        if(file.size <= 1000*400) {
            ...
            return res.status(400).json({msg: "Archivo muy chico."})
        }
        ...
        cloudinary.v2.uploader.upload(file.tempFilePath, 
            {folder: "PPB/Banners"}, async(err,result) => {
            if(err) 
                throw err; 
...
router.put('/bannerUpdate', auth, authAdmin, (req,res) => {

})


// Delete image (only admin)
router.post('/destroy/banner', auth, authAdmin, (req,res) =>{
    try{
